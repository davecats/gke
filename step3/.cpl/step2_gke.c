...
int step2_gke_1h;
int step2_gke_2h;
ssize_t step2_gke_3i;
ssize_t step2_gke_4i;
void gkemem_free(void * ptr){munmap((char*)ptr,step2_gke_4i);}struct freefunc gkemem_f;char *gkemem_;
void gkememM_free(void * ptr){munmap((char*)ptr,step2_gke_4i);}struct freefunc gkememM_f;char *gkememM_;
...
step2_gke_1h=(*(int *)(((ny_/2)+1)*(ssize_t)sizeof(int)+startpos_))-1;
step2_gke_2h=mz_-1;
step2_gke_3i=(ssize_t)sizeof(struct GKETERMS_)*(step2_gke_2h+1);
step2_gke_4i=step2_gke_3i*(step2_gke_1h+1);

gkemem_=mmap(NULL,step2_gke_4i,PROT_READ|PROT_WRITE,MAP_SHARED|MAP_ANON,-1,0);if(gkemem_==MAP_FAILED)errmalloc();atblockexit(gkemem_f,gkemem_free,(void*)gkemem_);memset(gkemem_,0,step2_gke_4i);
gkememM_=mmap(NULL,step2_gke_4i,PROT_READ|PROT_WRITE,MAP_SHARED|MAP_ANON,-1,0);if(gkememM_==MAP_FAILED)errmalloc();atblockexit(gkememM_f,gkememM_free,(void*)gkememM_);memset(gkememM_,0,step2_gke_4i);derivatives:;  {int ix_=0  ;while(ix_<=mx_-1  ){
        register int md7;
 fprintf(stdout,"""ix = ""\t%d\t"" of ""\t%d\t"": Reading data...""" ,ix_  ,mx_-1 );putc('\n',stdout);  t1_=wallclock_1wallclock(); 
          {int _5i_=step2_gke_1h;do{{if((ssize_t)sizeof(struct GKETERMS_)==(ssize_t)sizeof(struct GKETERMS_)&&(ssize_t)sizeof(struct GKETERMS_)==(ssize_t)sizeof(struct GKETERMS_)){if(!(fseeko(gkedata_,_5i_*(off_t)gkedata_113i+ix_*(off_t)gkedata_112i,SEEK_SET)==0&&fread(_5i_*step2_gke_3i+gkemem_,(ssize_t)sizeof(struct GKETERMS_),step2_gke_2h+1,gkedata_)==step2_gke_2h+1))ioerr(gkedata_);}else{  {int _6i_=step2_gke_2h;do{{if(!(fseeko(gkedata_,_6i_*(off_t)(ssize_t)sizeof(struct GKETERMS_)+_5i_*(off_t)gkedata_113i+ix_*(off_t)gkedata_112i ,SEEK_SET)==0&&fread((struct GKETERMS_ *)(_6i_*(ssize_t)sizeof(struct GKETERMS_)+_5i_*step2_gke_3i+gkemem_),(ssize_t)sizeof(struct GKETERMS_),1,gkedata_)==1))ioerr(gkedata_);}_6i_--;}while(_6i_>=0);}};}_5i_--;}while(_5i_>=0);}    {int _8i_=step2_gke_1h;do{{if((ssize_t)sizeof(struct GKETERMS_)==(ssize_t)sizeof(struct GKETERMS_)&&(ssize_t)sizeof(struct GKETERMS_)==(ssize_t)sizeof(struct GKETERMS_)){if(!(fseeko(gkedata_,_8i_*(off_t)gkedata_113i+(md7=(mx_-ix_)%mx_,md7>=0?md7:md7+mx_)*(off_t)gkedata_112i,SEEK_SET)==0&&fread(_8i_*step2_gke_3i+gkememM_,(ssize_t)sizeof(struct GKETERMS_),step2_gke_2h+1,gkedata_)==step2_gke_2h+1))ioerr(gkedata_);}else{  {int _9i_=step2_gke_2h;do{{if(!(fseeko(gkedata_,_9i_*(off_t)(ssize_t)sizeof(struct GKETERMS_)+_8i_*(off_t)gkedata_113i+(md7=(mx_-ix_)%mx_,md7>=0?md7:md7+mx_)*(off_t)gkedata_112i ,SEEK_SET)==0&&fread((struct GKETERMS_ *)(_9i_*(ssize_t)sizeof(struct GKETERMS_)+_8i_*step2_gke_3i+gkememM_),(ssize_t)sizeof(struct GKETERMS_),1,gkedata_)==1))ioerr(gkedata_);}_9i_--;}while(_9i_>=0);}};}_8i_--;}while(_8i_>=0);}  
        t2_=wallclock_1wallclock();    fprintf(stdout,"""ix = ""\t%d\t"" of ""\t%d\t"":Reading took ""\t%g\t"" seconds""" ,ix_  ,mx_-1  ,t2_-t1_ );putc('\n',stdout);
         fprintf(stdout,"""ix = ""\t%d\t"" of ""\t%d\t"":Computing derivatives... """ ,ix_  ,mx_-1 );putc('\n',stdout);  t1_=wallclock_1wallclock(); 
         {int iz_=0  ;while(iz_<=mz_-1){
          /*! y2-derivatives*/
           {int IY1_= - 1  ;while(IY1_<=(ny_ / 2)){
             {int IY2_=IY1_  ;while(IY2_<=ny_-IY1_){
              int step2_gke_10j;
/*j=step2_gke_10j*/

              step2_gke_10j=(( IY2_== - 1  ?2  :( IY2_==0  ?1  :( IY2_==ny_  ? - 1  :( IY2_==ny_+1  ? - 2  :0)))));
{
                struct gkedata_s5 *step2_gke_11w;
double step2_gke_14;
double step2_gke_17;
step2_gke_11w=(struct gkedata_s5 *)(IY2_*(ssize_t)sizeof(struct gkedata_s5)+derivs_);
 step2_gke_14=0.; {int i_= - 2  ;do{{register int md12;
register int md13;
(*&step2_gke_14)+=0.5*ni_*(*(double *)(i_*(ssize_t)sizeof(double)+(*step2_gke_11w).d1_-gkedata_6st))*(*((( (IY2_+i_+step2_gke_10j)<(IY1_)  ?((struct GKETERMS_ *)((md12=(mz_-iz_)%mz_,md12>=0?md12:md12+mz_)*(ssize_t)sizeof(struct GKETERMS_)+((*(int*)((IY2_+i_+step2_gke_10j)*(ssize_t)sizeof(int)+startpos_))+IY1_-(IY2_+i_+step2_gke_10j))*step2_gke_3i+gkememM_))  :( (IY2_+i_+step2_gke_10j)>ny_-(IY1_)  ?((struct GKETERMS_ *)((md13=(mz_-iz_)%mz_,md13>=0?md13:md13+mz_)*(ssize_t)sizeof(struct GKETERMS_)+((*(int*)((ny_-(IY2_+i_+step2_gke_10j))*(ssize_t)sizeof(int)+startpos_))+(ny_-IY1_)-(ny_-(IY2_+i_+step2_gke_10j)))*step2_gke_3i+gkememM_))  :((struct GKETERMS_ *)(iz_*(ssize_t)sizeof(struct GKETERMS_)+((*(int*)(IY1_*(ssize_t)sizeof(int)+startpos_))+(IY2_+i_+step2_gke_10j)-IY1_)*step2_gke_3i+gkemem_))))))).scaleENER_ ;}i_+=1;}while(i_<=2);}(*(struct GKETERMS_ *)(iz_*(ssize_t)sizeof(struct GKETERMS_)+((*(int*)(IY1_*(ssize_t)sizeof(int)+startpos_))+IY2_-IY1_)*step2_gke_3i+gkemem_)).phiC_    -=step2_gke_14;
                     step2_gke_17=0.; {int i_= - 2  ;do{{register int md15;
register int md16;
(*&step2_gke_17)+=ni_*(*(double *)(i_*(ssize_t)sizeof(double)+(*step2_gke_11w).d1_-gkedata_6st))*(*((( (IY2_+i_+step2_gke_10j)<(IY1_)  ?((struct GKETERMS_ *)((md15=(mz_-iz_)%mz_,md15>=0?md15:md15+mz_)*(ssize_t)sizeof(struct GKETERMS_)+((*(int*)((IY2_+i_+step2_gke_10j)*(ssize_t)sizeof(int)+startpos_))+IY1_-(IY2_+i_+step2_gke_10j))*step2_gke_3i+gkememM_))  :( (IY2_+i_+step2_gke_10j)>ny_-(IY1_)  ?((struct GKETERMS_ *)((md16=(mz_-iz_)%mz_,md16>=0?md16:md16+mz_)*(ssize_t)sizeof(struct GKETERMS_)+((*(int*)((ny_-(IY2_+i_+step2_gke_10j))*(ssize_t)sizeof(int)+startpos_))+(ny_-IY1_)-(ny_-(IY2_+i_+step2_gke_10j)))*step2_gke_3i+gkememM_))  :((struct GKETERMS_ *)(iz_*(ssize_t)sizeof(struct GKETERMS_)+((*(int*)(IY1_*(ssize_t)sizeof(int)+startpos_))+(IY2_+i_+step2_gke_10j)-IY1_)*step2_gke_3i+gkemem_))))))).scaleENER_ ;}i_+=1;}while(i_<=2);}(*(double *)(2*(ssize_t)sizeof(double)+(*(struct GKETERMS_*)(iz_*(ssize_t)sizeof(struct GKETERMS_)+((*(int*)(IY1_*(ssize_t)sizeof(int)+startpos_))+IY2_-IY1_)*step2_gke_3i+gkemem_)).phiR_-((ssize_t)sizeof(double))) )-=step2_gke_17;
            }IY2_+=1;};}
          IY1_+=1;};}
          /*! y1-derivatives*/
           {int IY1_= - 1  ;while(IY1_<=(ny_ / 2)){
...
/*j=step2_gke_18j*/
            ...
            step2_gke_18j=(( IY1_== - 1  ?2  :( IY1_==0  ?1  :( IY1_==ny_  ? - 1  :( IY1_==ny_+1  ? - 2  :0)))));
 {int IY2_=IY1_  ;while(IY2_<=ny_-IY1_){
              {
                struct gkedata_s5 *step2_gke_19w;
double step2_gke_23;
double step2_gke_27;
step2_gke_19w=(struct gkedata_s5 *)(IY1_*(ssize_t)sizeof(struct gkedata_s5)+derivs_);
 step2_gke_23=0.; {int i_= - 2  ;do{{register int md20;
register int md21;
register int md22;
(*&step2_gke_23)+=0.5*ni_*(*(double *)(i_*(ssize_t)sizeof(double)+(*step2_gke_19w).d1_-gkedata_6st))*(*((( (IY1_+i_+step2_gke_18j)>(ny_ / 2 ) ?(( (IY2_)==ny_-(IY1_+i_+step2_gke_18j)  ?((struct GKETERMS_ *)((md20=(mz_-iz_)%mz_,md20>=0?md20:md20+mz_)*(ssize_t)sizeof(struct GKETERMS_)+((*(int*)(IY2_*(ssize_t)sizeof(int)+startpos_))+(IY1_+i_+step2_gke_18j)-IY2_)*step2_gke_3i+gkememM_))  :((struct GKETERMS_ *)(iz_*(ssize_t)sizeof(struct GKETERMS_)+((*(int*)((ny_-(IY1_+i_+step2_gke_18j))*(ssize_t)sizeof(int)+startpos_))+(ny_-IY2_)-(ny_-(IY1_+i_+step2_gke_18j)))*step2_gke_3i+gkemem_))))  :((( ((IY2_))<((IY1_+i_+step2_gke_18j))  ?((struct GKETERMS_ *)((md21=(mz_-iz_)%mz_,md21>=0?md21:md21+mz_)*(ssize_t)sizeof(struct GKETERMS_)+((*(int*)(IY2_*(ssize_t)sizeof(int)+startpos_))+(IY1_+i_+step2_gke_18j)-IY2_)*step2_gke_3i+gkememM_))  :( ((IY2_))>ny_-((IY1_+i_+step2_gke_18j))  ?((struct GKETERMS_ *)((md22=(mz_-iz_)%mz_,md22>=0?md22:md22+mz_)*(ssize_t)sizeof(struct GKETERMS_)+((*(int*)((ny_-IY2_)*(ssize_t)sizeof(int)+startpos_))+(ny_-(IY1_+i_+step2_gke_18j))-(ny_-IY2_))*step2_gke_3i+gkememM_))  :((struct GKETERMS_ *)(iz_*(ssize_t)sizeof(struct GKETERMS_)+((*(int*)((IY1_+i_+step2_gke_18j)*(ssize_t)sizeof(int)+startpos_))+IY2_-(IY1_+i_+step2_gke_18j))*step2_gke_3i+gkemem_)))))))))).scaleENER_ ;}i_+=1;}while(i_<=2);}(*(struct GKETERMS_ *)(iz_*(ssize_t)sizeof(struct GKETERMS_)+((*(int*)(IY1_*(ssize_t)sizeof(int)+startpos_))+IY2_-IY1_)*step2_gke_3i+gkemem_)).phiC_    -=step2_gke_23;
                     step2_gke_27=0.; {int i_= - 2  ;do{{register int md24;
register int md25;
register int md26;
(*&step2_gke_27)+=ni_*(*(double *)(i_*(ssize_t)sizeof(double)+(*step2_gke_19w).d1_-gkedata_6st))*(*((( (IY1_+i_+step2_gke_18j)>(ny_ / 2 ) ?(( (IY2_)==ny_-(IY1_+i_+step2_gke_18j)  ?((struct GKETERMS_ *)((md24=(mz_-iz_)%mz_,md24>=0?md24:md24+mz_)*(ssize_t)sizeof(struct GKETERMS_)+((*(int*)(IY2_*(ssize_t)sizeof(int)+startpos_))+(IY1_+i_+step2_gke_18j)-IY2_)*step2_gke_3i+gkememM_))  :((struct GKETERMS_ *)(iz_*(ssize_t)sizeof(struct GKETERMS_)+((*(int*)((ny_-(IY1_+i_+step2_gke_18j))*(ssize_t)sizeof(int)+startpos_))+(ny_-IY2_)-(ny_-(IY1_+i_+step2_gke_18j)))*step2_gke_3i+gkemem_))))  :((( ((IY2_))<((IY1_+i_+step2_gke_18j))  ?((struct GKETERMS_ *)((md25=(mz_-iz_)%mz_,md25>=0?md25:md25+mz_)*(ssize_t)sizeof(struct GKETERMS_)+((*(int*)(IY2_*(ssize_t)sizeof(int)+startpos_))+(IY1_+i_+step2_gke_18j)-IY2_)*step2_gke_3i+gkememM_))  :( ((IY2_))>ny_-((IY1_+i_+step2_gke_18j))  ?((struct GKETERMS_ *)((md26=(mz_-iz_)%mz_,md26>=0?md26:md26+mz_)*(ssize_t)sizeof(struct GKETERMS_)+((*(int*)((ny_-IY2_)*(ssize_t)sizeof(int)+startpos_))+(ny_-(IY1_+i_+step2_gke_18j))-(ny_-IY2_))*step2_gke_3i+gkememM_))  :((struct GKETERMS_ *)(iz_*(ssize_t)sizeof(struct GKETERMS_)+((*(int*)((IY1_+i_+step2_gke_18j)*(ssize_t)sizeof(int)+startpos_))+IY2_-(IY1_+i_+step2_gke_18j))*step2_gke_3i+gkemem_)))))))))).scaleENER_ ;}i_+=1;}while(i_<=2);}(*(double *)(2*(ssize_t)sizeof(double)+(*(struct GKETERMS_*)(iz_*(ssize_t)sizeof(struct GKETERMS_)+((*(int*)(IY1_*(ssize_t)sizeof(int)+startpos_))+IY2_-IY1_)*step2_gke_3i+gkemem_)).phiR_-((ssize_t)sizeof(double))) )+=step2_gke_27;
            }IY2_+=1;};}
          IY1_+=1;};}
        iz_+=1;};}
        t2_=wallclock_1wallclock();    fprintf(stdout,"""ix = ""\t%d\t"" of ""\t%d\t"":Computing took ""\t%g\t"" seconds""" ,ix_  ,mx_-1  ,t2_-t1_ );putc('\n',stdout);
         fprintf(stdout,"""ix = ""\t%d\t"" of ""\t%d\t"":Writing data...""" ,ix_  ,mx_-1 );putc('\n',stdout);  t1_=wallclock_1wallclock(); 
           {int _28i_=gkedata_109h;do{{if((ssize_t)sizeof(struct GKETERMS_)==(ssize_t)sizeof(struct GKETERMS_)&&(ssize_t)sizeof(struct GKETERMS_)==(ssize_t)sizeof(struct GKETERMS_)){if(!(fseeko(gkedata_,_28i_*(off_t)gkedata_113i+ix_*(off_t)gkedata_112i,SEEK_SET)==0&&fwrite(_28i_*step2_gke_3i+gkemem_,(ssize_t)sizeof(struct GKETERMS_),gkedata_111h+1,gkedata_)==gkedata_111h+1))ioerr(gkedata_);}else{  {int _29i_=gkedata_111h;do{{if(!(fseeko(gkedata_,_29i_*(off_t)(ssize_t)sizeof(struct GKETERMS_)+_28i_*(off_t)gkedata_113i+ix_*(off_t)gkedata_112i,SEEK_SET)==0&&fwrite((struct GKETERMS_*)(_29i_*(ssize_t)sizeof(struct GKETERMS_)+_28i_*step2_gke_3i+gkemem_),(ssize_t)sizeof(struct GKETERMS_),1,gkedata_)==1))ioerr(gkedata_);}_29i_--;}while(_29i_>=0);}};}_28i_--;}while(_28i_>=0);}
        t2_=wallclock_1wallclock();    fprintf(stdout,"""ix = ""\t%d\t"" of ""\t%d\t"":Writing took ""\t%g\t"" seconds""" ,ix_  ,mx_-1  ,t2_-t1_ );putc('\n',stdout);
ix_+=1;};}derivatives_exit:;
printf("\r                                            \r");
errfclose(&gkedata_);
